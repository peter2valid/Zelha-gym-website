  ...
  <div>
    ...
    <section class="relative py-28 overflow-hidden">
      ...
      <div class="relative z-10 max-w-4xl mx-auto px-4 text-center">
        ...
        <h1 class="text-white uppercase mb-6" style="font-family: 'Bebas Neue', Impact, sans-serif; font-size: clamp(3rem, 8vw, 6rem); line-height: 0.95;">Join via <span class="text-primary">WhatsApp</span></h1>
        <p class="text-gray-300 text-lg max-w-2xl mx-auto">
          We handle all memberships and inquiries directly through WhatsApp for a faster, more personal experience. Fill out your details and we'll start your registration immediately.
        </p>
      </div>
    </section>
    ...
    <section class="py-16 bg-secondary">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        <div class="grid lg:grid-cols-2 gap-12 items-start">
          <!-- Sign-up Form -->
          <div class="card-dark p-8 sm:p-10 border-primary/20">
            <h2 class="text-white font-black uppercase tracking-wide text-2xl mb-6">Registration Inquiry</h2>
            <form @submit.prevent="submitForm" class="space-y-5">
              <div class="grid sm:grid-cols-2 gap-5">
                <div>
                  <label class="block text-gray-400 text-xs font-black uppercase tracking-widest mb-2">Full Name</label>
                  <input type="text" v-model="form.name" class="w-full px-4 py-3 bg-secondary text-gray-100 border border-gray-800 focus:border-primary focus:outline-none transition-colors" placeholder="e.g. John Doe" required />
                </div>
                <div>
                  <label class="block text-gray-400 text-xs font-black uppercase tracking-widest mb-2">Phone Number</label>
                  <input type="tel" v-model="form.phone" class="w-full px-4 py-3 bg-secondary text-gray-100 border border-gray-800 focus:border-primary focus:outline-none transition-colors" placeholder="e.g. 0712 345 678" required />
                </div>
              </div>
              <div>
            ...
            <p class="text-gray-500 text-[10px] mt-6 text-center uppercase tracking-widest leading-relaxed">This will open a chat with us on WhatsApp where we can finalize your membership.</p>
          </div>

          <!-- Benefits/CTA -->
          <div class="lg:sticky lg:top-32">
            <h2 class="section-heading mb-6">Why Join Us?</h2>
            <div class="space-y-6 mb-10">
              <div v-for="benefit in joinBenefits" :key="benefit.title" class="flex gap-4">
            ...
            <div class="p-8 bg-secondary-light border border-gray-800 rounded-sm">
              <h3 class="text-white font-black uppercase tracking-wide text-lg mb-2">Prefer to just chat?</h3>
              <p class="text-gray-400 text-sm mb-6">You can also skip the form and message us directly with any questions you have.</p>
              <a :href="whatsAppLink()" target="_blank" class="btn-outline w-full py-4 text-sm font-black uppercase">
...
function submitForm() {
  const message = encodeURIComponent(
    `Hi Zelha Fitness! I'd like to join. \n\n*Name:* ${form.name}\n*Phone:* ${form.phone}\n*Interest:* ${form.package}${form.notes ? '\n*Notes:* ' + form.notes : ''}`
  )
